<script lang="ts">
	...
	export let asistentes: AsistenteDeCurso[];
...
{#if asistentes}
	...
	{:else}
		<div class="flex flex-col gap-2">
			{#each makeArraySearchable( asistentes.map(
					({ aprobado, docente }) => ({ ...docente, aprobado })
				), [, 'nombre', 'apellido_paterno', 'apellido_materno'], filterText ) as asistente (asistente.id)}
